...
func TestProp_OnValue_fires(t *testing.T) {
	p := NewProp(0)
	fired := false
	p.OnValue(5, func() { fired = true })
	p.Set(5)
	if !fired {
		t.Fatal("expected OnValue callback to fire")
	}
}

func TestProp_OnValue_unsub(t *testing.T) {
	p := NewProp(0)
	count := 0
	unsub := p.OnValue(5, func() { count++ })
	p.Set(5)
	if count != 1 {
...
func TestPropEq_customEquality(t *testing.T) {
	// eq ignores sign
	p := NewPropEq(1, func(a, b int) bool { return a == b || -a == b })
	count := 0
	p.OnChange(func(int) { count++ })
	p.Set(-1) // considered equal
	if count != 0 {
		t.Fatalf("expected no fire for sign-equal value, got count=%d", count)
	}
	p.Set(2)
	if count != 1 {
		t.Fatalf("expected count 1 after distinct value, got %d", count)
	}
}
